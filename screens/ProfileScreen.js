...
return Alert.alert(
  ...
  [
    ...
    { text: "OK", onPress: () => {navigation.navigate("GeneralInfo", { data })
...
useEffect(()=>{
  ...
  if(data?.birthday_millis  && data?.dob 
    && data?.height  && data?.weight &&
    data?.place && data?.state && data?.pincode && data?.city
    ){
      
    }
    else if(showAlertBox ) {
      showAlert(data)
    }}
},[data])
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator ={false}>
        <View style={styles.ProfileCard} onPress={() => GetProfile()}>
          ...
          <View>
            <View style={styles.profname}>
              ...
              <Text style={styles.PatientName}>{data?.patient_name ? data?.patient_name.charAt(0).toUpperCase() + data?.patient_name.slice(1): data?.patient_name}</Text>
        ...
        <View style={[styles.row1, { marginBottom: 20 }]}>
          ...
          {data.health_ids && data.health_ids.length >0 && <TouchableOpacity
            onPress={() => navigation.navigate("HealthIdsList", {data : data})}
            style={styles.optionCards}
          >
            <View style={styles.expoalign}>
              <MaterialCommunityIcons
            ...
            <View style={styles.textstyle}>
              <Text style={styles.smallcardtext}>Health ID's</Text>
            </View>
          </TouchableOpacity>}
        </View>
      </ScrollView>
